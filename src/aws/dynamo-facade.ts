...
import {
    ...
    getRecipesByIdList,
    getUsers,
    postRecipe, postRecipeUserRelationship
} from "./dynamo";
import {dbResponseToRecipe, dbResponseToRecipeIds, dbResponseToRecipeList, dbResponseToUsers} from "./dynamo-utils";

export const getAllUserRecipes = async (userId: string, accessToken: string): Promise<Recipe[]> => {
    //get recipeIds
    ...
    for (let i = 0; i < recipeIdArrays.length; i++) {
        ...
        const recipeList = dbResponseToRecipeList(recipeListResponse.Responses?.['recipeBook-recipe']);
...
export const getSingleRecipe = async (recipeId: string, accessToken: string): Promise<Recipe> => {
    const recipeResponse = await getRecipeById(recipeId, accessToken);
    return dbResponseToRecipe(recipeResponse.Item);
};

export const postNewRecipe = async (recipeBase: RecipeBase, userId: string, accessToken: string): Promise<string> => {
    const recipe: Recipe = {...recipeBase, recipeId: uuidv4()};

    //post the recipe
    await postRecipe(recipe, accessToken);

    await putNewRecipeRelationship(userId, recipe.recipeId as string, accessToken);

    return recipe.recipeId as string;
};

export const updateRecipe = async (recipe: Recipe, userId: string, accessToken: string): Promise<string> => {
...
export const deleteRecipeRelationship = async (recipe: Recipe, userId: string, accessToken: string): Promise<void> => {
...
export const deleteRecipe = async (recipe: Recipe, userId: string, accessToken: string): Promise<void> => {
    await deleteRecipeRelationship(recipe, userId, accessToken);
    await deleteRecipeActual(recipe.recipeId, accessToken);
};

export const getAllUserRecipeIds = async (userId: string, accessToken: string): Promise<string[]> => {
    const recipeIdResponse = await getRecipeIdsByUser(userId, accessToken);
    return dbResponseToRecipeIds(recipeIdResponse.Items?.[0]);
};

export const putNewRecipeRelationship = async (userId: string, recipeId: string, accessToken: string) => {
    //get the user
    const recipeIdsResponse = await getRecipeIdsByUser(userId, accessToken);
