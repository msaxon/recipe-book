...
import {DBBatchItem, DBDeleteItem, DBGetItem, DBPutItem, DBQuery, DBScan} from "./dynamo-types";
...
/*** CRUD ***/
export const getRecipeIdsByUser = async (userId: string, accessToken: string): Promise<DBQuery> => {
...
export const getRecipesByIdList = async (idList: string[], accessToken: string): Promise<DBBatchItem> => {
    const db = await setupAuth(accessToken);
...
export const getRecipeById = async (recipeId: string, accessToken: string): Promise<DBGetItem> => {
    const db = await setupAuth(accessToken);
    const getRecipeIdParams = {
        Key: {
            recipeId: {
...
export const postRecipe = async (recipe: Recipe, accessToken: string): Promise<DBPutItem>  => {
    const db = await setupAuth(accessToken);
...
export const postRecipeUserRelationship = async (recipeIds: string[], userId: string, accessToken: string): Promise<DBPutItem> => {
    const db = await setupAuth(accessToken);
    const putUserParams = {
        TableName: 'recipeBook-userRecipePair',
        Item: {
...
export const deleteRecipeActual = async (recipeId: string, accessToken: string): Promise<DBDeleteItem> => {
    const db = await setupAuth(accessToken);
